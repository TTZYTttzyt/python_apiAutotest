...
#define MPU_NSS_LOW HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET)
#define MPU_NSS_HIGH HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET)
...
void imu_tempctrl(void)
{
	PID_SingleCalc(&TempPID,45,imu.temp);
	LIMIT(TempPID.output,0,10000);
	TIM3->CCR2 = TempPID.output;
}

void IMU_Init(void)
{
	PID_Init(&TempPID,50000,10,0,5000,10000);
